...
# Artifact loading
# -----------------------
from utils.preprocess import preprocess_for_classification  # needed for legacy pickles


def clean_text_wrapper(text_series):
    """
    Legacy wrapper sometimes referenced by the pickled Pipeline (saved when training script ran as __main__).

    IMPORTANT:
    - Keep this EXACT name at top-level so pickle can resolve __main__.clean_text_wrapper.
    - Signature supports Series / DataFrame / list inputs used by sklearn pipelines.
    """
...
def _tokenize_simple(text: str) -> List[str]:
    # We *do not* use preprocess_for_correction here because the spellchecker already
...
def _iter_tok_ws(text: str, strict_preprocess: bool):
    """
    Yield (tok, ws) pairs.
    - strict_preprocess=True: uses preprocess_for_correction() pieces to preserve separators.
    - strict_preprocess=False: falls back to simple split (NOT identical to app).
    """
    if strict_preprocess:
        from utils.preprocess import preprocess_for_correction  # local import
        pieces = preprocess_for_correction(text)

        # Backward-compat in case it returns a string or list[str]
        if isinstance(pieces, str):
            for tok in pieces.split():
                yield tok, " "
            return

        for p in pieces:
            if isinstance(p, dict):
                yield str(p.get("text", "")), str(p.get("ws", ""))
            else:
                # old behavior: list[str]
                yield str(p), " "
    else:
...
def correct_text_conservative(
    ...
    """
    Conservative: preserve separators; correct ONLY OOV (non-word) + special corrections.
    Mirrors the App reconstruction when strict_preprocess=True.
    """
    if text is None:
        return ""
    text = str(text)
    ...
    for tok, ws in _iter_tok_ws(text, strict_preprocess):
        if not tok:
            continue

        # keep non-alphaish tokens (punctuation/numbers) as-is, but preserve ws
...
def correct_text_full(
    ...
    if strict_preprocess:
        from utils.preprocess import preprocess_for_correction
        toks = preprocess_for_correction(text)
...
def main():
    ...
    ap.add_argument("--bootstrap", type=int, default=5000, help="Bootstrap samples for delta CI (paper default=5000; 0=off).")
    ...
    ap.add_argument("--strict_preprocess", action="store_true", help="Use preprocess_for_correction tokenization (slower, closer to app).")
    ...
    ap.add_argument("--emit_trace", action="store_true",
                    help="Emit JSONL audit traces for Restored and Safety runs (Paper Table III).")
    args = ap.parse_args()
    ...
    # NOTE: Both paths are paper-compliant — spell.correct() now always
    # applies safety gates (check_protection_gates) + margin gates (δ, δ_rw)
    # via the unified _correct_core() in model.py (Paper Algorithm 1).

    # Trace-aware correction using core spellchecker (Paper Table III)
    def correct_text_with_trace(spell_checker, text, doc_id=""):
        """Use core correct_text_with_trace for JSONL trace emission."""
        corrected, traces, summary = spell_checker.correct_text_with_trace(text, doc_id=doc_id)
        return corrected, traces, summary
    ...
    all_restored_traces = []  # for JSONL output
    for i, t in enumerate(noisy_texts, start=1):
        if t in repaired_cache:
            repaired_texts.append(repaired_cache[t])
        else:
            if args.emit_trace:
                repaired, traces, summary = correct_text_with_trace(spell, t, doc_id=f"restored_{i}")
                all_restored_traces.extend([tr.to_dict() for tr in traces])
                all_restored_traces.append(summary.to_dict())
            else:
    ...
    report: Dict[str, Any] = {
        ...
        "spellchecker_params": {
            "delta": float(spell.delta),
            "delta_rw": float(spell.delta_rw),
            "artifact_version": str(getattr(spell, "artifact_version", "")),
        },
        "metrics": {
    ...
    if args.output_rows:
        ...
        rows = pd.DataFrame({
            ...
            "pred_safe": pred_safe,
            "label_flip_noisy_restored": [1 if pn != pr else 0 for pn, pr in zip(pred_noisy, pred_repaired)],
            "label_flip_clean_safety": [1 if pc != ps else 0 for pc, ps in zip(pred_clean, pred_safe)],
    ...
    # Add artifact_version to report
    try:
        report["artifact_version"] = spell.artifact_version
    except Exception:
        pass
    ...
    # Save trace JSONL files if --emit_trace
    if args.emit_trace and args.output_report:
        trace_dir = Path(args.output_report).parent
        if all_restored_traces:
            trace_path = trace_dir / "trace_restored.jsonl"
            with open(trace_path, "w", encoding="utf-8") as tf:
                for tr_dict in all_restored_traces:
                    tf.write(json.dumps(tr_dict, ensure_ascii=False) + "\n")
